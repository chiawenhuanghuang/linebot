...
def sendCarousel_all(event,result_list):  #轉盤樣板
    try:
        ...
        for i in range(3):
            print(result_list[i][1]," ",source_worksheet(result_list[i])," ",result_list[i][2]," ",result_list[i][3]," ")
            column.append(
                CarouselColumn
                (
        ...
        print("column",column,"--------------------")
        message_a = TemplateSendMessage(
            alt_text='所有房間訊息',
            template=CarouselTemplate(
                columns=column
            )
        )
        print("message",message_a)
        line_bot_api.push_message("Ucaabcb546c73f6bf6cd5ee27a3ce3848", message_a)
